---
name: merge-pr
description: Merge an open GitHub pull request by PR number using gh CLI with safety checks. Use when the user asks to merge a PR and provides a PR number.
---

# Merge PR

## Purpose
Merge an open pull request when the user provides a PR number argument.
...
4. Merge:
   - Default method: squash merge with branch deletion.
   - Command:
     - `gh pr merge <pr_number> --squash --delete-branch`
   - If user requests a different method, use `--merge` or `--rebase` instead.
5. Verify:
